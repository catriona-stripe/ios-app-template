 ...
 */
static NSString *const SCPSDKVersion = @"1.1.0-b1";

@class SCPCancelable,
SCPConnectConfiguration,
...
/**
 ...
 Currently the `connectConfig` is only utilized by `verifoneP400` and will
 be ignored if passed while attempting to connect to a `chipper2X`.

 @see https://stripe.com/docs/terminal/readers/connecting

 @param reader          The reader to connect to. This should be a reader
 recently returned to the `didUpdateDiscoveredReaders:` method.
 @param connectConfig   The connect configuration for options while connecting
 to a reader. See `SCPConnectConfiguration`for more details.
 @param completion      The completion block called when the command completes.
 */
- (void)connectReader:(SCPReader *)reader
        connectConfig:(nullable SCPConnectConfiguration *) connectConfig
           completion:(SCPReaderCompletionBlock)completion NS_SWIFT_NAME(connectReader(_:connectConfig:completion:));
...
/**
 ...
 Note this cannot be used with `verifoneP400` and will assert if called while
 connected to a device of type `verifoneP400`.
